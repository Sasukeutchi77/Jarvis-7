...
import { matchSecurityCode } from './tts-sanitizer';
...
function loadSettings(): Settings {
  const defaults: Settings = {
    ...
    voiceGreetingText: 'Oui, je vous écoute.',
  };
  try {
...
interface AppState {
  ...
  jarvisSecurityCode: string;
  jarvisAuthEnabled: boolean;
  jarvisIsLocked: boolean;
  userName: string;
  identityModalOpen: boolean;
  setIdentityModalOpen: (open: boolean) => void;
  setJarvisSecurityCode: (code: string) => void;
  setJarvisAuthEnabled: (enabled: boolean) => void;
  setUserName: (name: string) => void;
  unlockJarvis: (inputCodeOrSpeech: string) => boolean;
  lockJarvis: () => void;
  ...
  setMorningBriefing: (data: MorningBriefingData | null) => void;
}
...
export const useAppStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    // ── Security & Identity Confirmation ─────────────────────────────
    jarvisSecurityCode: localStorage.getItem('jarvis_security_code') || '4920',
    jarvisAuthEnabled: localStorage.getItem('jarvis_auth_enabled') !== 'false',
    jarvisIsLocked: localStorage.getItem('jarvis_auth_enabled') !== 'false',
    userName: localStorage.getItem('jarvis_user_name') || 'Monsieur',
    identityModalOpen: false,

    setIdentityModalOpen: (open: boolean) => set({ identityModalOpen: open }),
    setJarvisSecurityCode: (code: string) => {
      const clean = code.trim();
      localStorage.setItem('jarvis_security_code', clean);
      set({ jarvisSecurityCode: clean });
    },
    setJarvisAuthEnabled: (enabled: boolean) => {
      localStorage.setItem('jarvis_auth_enabled', String(enabled));
      set({ jarvisAuthEnabled: enabled, jarvisIsLocked: enabled ? get().jarvisIsLocked : false });
    },
    setUserName: (name: string) => {
      const clean = name.trim();
      localStorage.setItem('jarvis_user_name', clean);
      set({ userName: clean });
    },
    unlockJarvis: (inputCodeOrSpeech: string) => {
      const masterCode = get().jarvisSecurityCode;
      const isValid = matchSecurityCode(inputCodeOrSpeech, masterCode);
      if (isValid) {
        set({ jarvisIsLocked: false, identityModalOpen: false });
        get().addLogEntry({
          timestamp: Date.now(),
          level: 'info',
          category: 'server',
          message: `Identité confirmée pour ${get().userName}. Accès total aux fonctions déverrouillé.`,
        });
        return true;
      } else {
        get().addLogEntry({
          ...
          level: 'warn',
          category: 'server',
          message: `Échec d'authentification vocale/PIN : tentative non autorisée bloquée.`,
        });
        return false;
      }
    },
    lockJarvis: () => {
      set({ jarvisIsLocked: true });
      get().addLogEntry({
        timestamp: Date.now(),
        level: 'info',
        category: 'server',
        message: 'Verrouillage de sécurité activé : confirmation d\'identité requise.',
      });
    },

    morningBriefing: null,
    setMorningBriefing: (data: MorningBriefingData | null) => set({ morningBriefing: data }),
